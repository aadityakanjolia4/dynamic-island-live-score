...
cors = CORS(
    ...
    resources={
        r"/score/*": {
            "origins": [r"^https://.+sanweb.info$", r"^https://.+mskian.com$"]
        }
    },
)
...
def scheduled_task():
    with app.app_context():
        get_id = MatchID.query.first()
        if get_id:
            print(f"Cron job running...{get_id}")
            # print(score(get_id))
            # return score(get_id)


scheduler.add_job(
    id="Scheduled Task", func=scheduled_task, trigger="interval", minutes=1
)
...
def add_match(id):
    ...
    print("!21212")
    existing_id = MatchID.query.filter_by(id=id).first()
    first_id = MatchID.query.first()
    print("Wewewewe", existing_id)
    if existing_id:
    ...
    # Add new user
        ...
        try:
            db.session.delete(first_id)
        except:
            pass
        print(id)
        print(type(id))
        new_user = MatchID(id=int(id), name="test")
...
headers = {"User-Agent": get_random_agent, "Cache-Control": "no-cache"}
    ...
    if id:
        ...
        try:
            update = (
                soup.find_all(
                    "div",
                    attrs={"class": "cb-col cb-col-100 cb-min-stts cb-text-complete"},
                )[0].text.strip()
                if soup.find_all(
                    "div",
                    attrs={"class": "cb-col cb-col-100 cb-min-stts cb-text-complete"},
                )
                else "Match Stats will Update Soon"
            )
            process = (
                soup.find_all("div", attrs={"class": "cb-text-inprogress"})[
                    0
                ].text.strip()
                if soup.find_all("div", attrs={"class": "cb-text-inprogress"})
                else "Match Stats will Update Soon"
            )
            noresult = (
                soup.find_all(
                    "div",
                    attrs={
                        "class": "cb-col cb-col-100 cb-font-18 cb-toss-sts cb-text-abandon"
                    },
                )[0].text.strip()
                if soup.find_all(
                    "div",
            ...
            stumps = (
                soup.find_all("div", attrs={"class": "cb-text-stumps"})[0].text.strip()
                if soup.find_all("div", attrs={"class": "cb-text-stumps"})
                else "Match Stats will Update Soon"
            )
            lunch = (
                soup.find_all("div", attrs={"class": "cb-text-lunch"})[0].text.strip()
                if soup.find_all("div", attrs={"class": "cb-text-lunch"})
                else "Match Stats will Update Soon"
            )
            inningsbreak = (
                ...
                if soup.find_all("div", attrs={"class": "cb-text-inningsbreak"})
                else "Match Stats will Update Soon"
            )
            tea = (
                soup.find_all("div", attrs={"class": "cb-text-tea"})[0].text.strip()
                if soup.find("div", attrs={"class": "cb-text-tea"})
                else "Match Stats will Update Soon"
            )
            rain_break = (
                soup.find_all("div", attrs={"class": "cb-text-rain"})[0].text.strip()
                if soup.find("div", attrs={"class": "cb-text-rain"})
                else "Match Stats will Update Soon"
            )
            wet_outfield = (
                ...
                if soup.find("div", attrs={"class": "cb-text-wetoutfield"})
                else "Match Stats will Update Soon"
            )
            ...
            live_score = (
                soup.find("span", attrs={"class": "cb-font-20 text-bold"}).text.strip()
                if soup.find("span", attrs={"class": "cb-font-20 text-bold"})
                else "Data Not Found"
            )
            title = (
                soup.find("h1", attrs={"class": "cb-nav-hdr cb-font-18 line-ht24"})
                .text.strip()
                .replace(", Commentary", "")
                if soup.find("h1", attrs={"class": "cb-nav-hdr cb-font-18 line-ht24"})
                else "Data Not Found"
            )
            run_rate = (
                soup.find_all("span", attrs={"class": "cb-font-12 cb-text-gray"})[0]
                .text.strip()
                .replace("CRR:\u00a0", "")
                if soup.find_all("span", attrs={"class": "cb-font-12 cb-text-gray"})
                else "Data Not Found"
            )
            batter_one = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-50"})[
                    1
                ].text.strip()
                if soup.find_all("div", attrs={"class": "cb-col cb-col-50"})
                else "Data Not Found"
            )
            batter_two = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-50"})[
            ...
            batter_one_run = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-10 ab text-right"})[
                    0
                ].text.strip()
                if soup.find_all(
                    "div", attrs={"class": "cb-col cb-col-10 ab text-right"}
                )
                else "Data Not Found"
            )
            batter_two_run = (
            ...
            batter_one_sr = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-14 ab text-right"})[
                    0
                ].text.strip()
                if soup.find_all(
                    "div", attrs={"class": "cb-col cb-col-14 ab text-right"}
                )
                else "Data Not Found"
            )
            batter_two_sr = (
            ...
            bowler_one_over = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-10 text-right"})[
                    4
                ].text.strip()
                if soup.find_all("div", attrs={"class": "cb-col cb-col-10 text-right"})
                else "Data Not Found"
            )
            bowler_two_over = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-10 text-right"})[
            ...
            bowler_one_eco = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-14 text-right"})[
                    2
                ].text.strip()
                if soup.find_all(
                    "div", attrs={"class": "cb-col cb-col-10 ab text-right"}
            ...
            bowler_one_wicket = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-8 text-right"})[
                    5
                ].text.strip()
                if soup.find_all("div", attrs={"class": "cb-col cb-col-8 text-right"})
                else "Data Not Found"
            )
            bowler_two_wicket = (
                soup.find_all("div", attrs={"class": "cb-col cb-col-8 text-right"})[
...
def update_match_id():
    ...
    return jsonify(
        {"message": "Match ID updated successfully", "current_match_id": new_match_id}
    )
    # return jsonify({
